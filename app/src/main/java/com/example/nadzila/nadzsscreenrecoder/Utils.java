...
class Utils {
    ...
    static MediaCodecInfo.CodecProfileLevel toProfileLevel(String str) {
        ...
        MediaCodecInfo.CodecProfileLevel res = null;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
            res = new MediaCodecInfo.CodecProfileLevel();
        }
    ...
    private static void initProfileLevels() {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
            fields = MediaCodecInfo.CodecProfileLevel.class.getFields();
        }
        for (Field f : fields) {
    ...
    private static void initColorFormatFields() {
        Field[] fields = new Field[0];
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN) {
            fields = MediaCodecInfo.CodecCapabilities.class.getFields();
        }
        for (Field f : fields) {
            if ((f.getModifiers() & (Modifier.STATIC | Modifier.FINAL)) == 0) {
                continue;
